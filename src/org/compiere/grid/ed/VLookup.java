	...
	private void actionText()
	{
		...
		if (id == -3)
		{
			...
			try
			{
				...
				{//05-10-2023 JLeyton Integracion con Codigo Generico
					String sql_temporal = "select max(M_PRODUCT_ID) from m_product_po where upper(vendorproductno) like upper('"+text+"')";
					log.finest("SQL CODIGO GENERICO==>"+sql_temporal);
					int id_temp=(int) DB.getSQLValue(null,sql_temporal);
					if(id_temp>0)
						id=id_temp;
				}
			}
