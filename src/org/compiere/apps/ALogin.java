...
//	private BorderLayout defaultLayout = new BorderLayout();
	...
	private void jbInit() throws Exception
	{
		...
		connectionPanel.add(languageCombo,          new GridBagConstraints(1, 5, 3, 1, 1.0, 0.0
			,GridBagConstraints.WEST, GridBagConstraints.HORIZONTAL, new Insets(5, 0, 5, 12), 0, 0));
		// @Trifon - end
		copy0Label.setText(AboutBox.OFBVersion);//faaguilar OFB version
	...
	private boolean tryConnection()
	{
		...
		for (KeyNamePair role : roles) {
			...
			if (role.getName().equals(iniDefault))
				iniValue = role;
		}
		if (iniValue != null)
	...
	private void roleComboChanged ()
	{
		...
		for (KeyNamePair client : clients) {
			...
			if (client.getName().equals(iniDefault))
				iniValue = client;
		}
		//	fini
		if (iniValue != null)
	...
	private void clientComboChanged()
	{
		...
		for (KeyNamePair org : orgs) {
			...
			if (org.getName().equals(iniDefault))
				orgValue = org;
	...
	private void orgComboChanged()
	{
		...
		if (whs != null)
		{
			...
			for (KeyNamePair wh : whs) {
				warehouseCombo.addItem(wh);
				if (wh.getName().equals(iniDefault))
					iniValue = wh;
			}
			if (iniValue != null)
