...
public class InOutCreateInvoice extends InOutCreateInvoiceAbstract
{
	...
	protected String doIt () throws Exception {
		log.info("M_InOut_ID=" + getRecord_ID()
			...
			//Se agrega 2 nuevos parametros
			+", DocType_ID= "+getDocTypeId()
			+", DocTypeTarget_ID= "+getDocTypeTargetId() );
		if (getRecord_ID() == 0)
		...
		if (!X_M_InOut.DOCSTATUS_Completed.equals(materialReceipt.getDocStatus()))
	...
	private MInvoice getCreateHeader(MInOut shipment)
	{
		...
		//Se agrega dos parametros para generar la C_invoice
		if(getDocTypeId() != 0)
