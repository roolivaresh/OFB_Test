...
public class CalloutPayment extends CalloutEngine
{
		...
		GridField mField, Object value)
	{
		...
		// se llama al valor absoluto del documento
		String sql = "SELECT C_BPartner_ID,C_Currency_ID," // 1..2
				+ " invoiceOpen(C_Invoice_ID, ?)," // 3 #1
				+ " invoiceDiscount(C_Invoice_ID,?,?), IsSOTrx, " // 4..5 #2/3
				+ " description " 
				+ "FROM C_Invoice WHERE C_Invoice_ID=?"; // #4
		if(OFBForward.isAbsolutePayAmtCallout())
			sql = "SELECT C_BPartner_ID,C_Currency_ID," // 1..2
			+ " abs(invoiceOpen(C_Invoice_ID, ?))," // 3 #1
			+ " abs(invoiceDiscount(C_Invoice_ID,?,?)), IsSOTrx, "// 4..5 #2/3
			+ " description " 
			+ "FROM C_Invoice WHERE C_Invoice_ID=?"; // #4
		...
		try
		{
			...
			if (rs.next ())
			{
				...
				// jleyton se setea la descripcion desde invoice
				mTab.setValue("Description",rs.getString("description"));
			}
		}
