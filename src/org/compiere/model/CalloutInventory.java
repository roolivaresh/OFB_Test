...
public class CalloutInventory extends CalloutEngine {
	...
	public String product(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value) {
		...
		if (InventoryLine != null && InventoryLine.intValue() != 0) {
			...
			// if product or locator or ASI (jleyton) has changed recalculate Book Qty
	...
	public String checkOnHand(Properties ctx, int WindowNo, GridTab mTab, GridField mField, Object value) {
		...
		if (line.getM_Product().isStocked() && line.getQtyInternalUse().signum() > 0) {
			...
			BigDecimal qtyOnHand = DB.getSQLValueBD(null,
					"SELECT bomQtyOnHand(M_Product_ID,?,?) FROM M_Product WHERE M_Product_ID=?",
					line.getM_Inventory().getM_Warehouse_ID(), 0, line.getM_Product_ID());
			if (qtyOnHand == null)
				qtyOnHand = Env.ZERO;
