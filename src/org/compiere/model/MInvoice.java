...
public class MInvoice extends X_C_Invoice implements DocAction , DocumentReversalEnabled
{
	...
	public MInvoice (MOrder order, int C_DocTypeTarget_ID, Timestamp invoiceDate)
	{
		...
		if (C_DocTypeTarget_ID <= 0)
		{
			...
			if (odt != null)
			{
				...
				if (C_DocTypeTarget_ID <= 0)
					throw new AdempiereException("@NotFound@ @C_DocTypeInvoice_ID@ - @C_DocType_ID@:"+odt.get_Translation(I_C_DocType.COLUMNNAME_Name));
			}
		}
	...
	public void setC_DocTypeTarget_ID (String DocBaseType)
	{
		...
		else
		{
			...
			boolean isSOTrx = X_C_DocType.DOCBASETYPE_ARInvoice.equals(DocBaseType)
	...
	public void setC_DocTypeTarget_ID ()
	{
		if (getC_DocTypeTarget_ID() > 0)
			return;
		if (isSOTrx())
			setC_DocTypeTarget_ID(X_C_DocType.DOCBASETYPE_ARInvoice);
		else
			setC_DocTypeTarget_ID(X_C_DocType.DOCBASETYPE_APInvoice);
	}	//	setC_DocTypeTarget_ID
	...
	public MInvoiceTax[] getTaxes (boolean requery)
	{
		...
		final String whereClause = I_C_InvoiceTax.COLUMNNAME_C_Invoice_ID+"=?";
	...
	public boolean isCreditMemo()
	{
		...
		return X_C_DocType.DOCBASETYPE_APCreditMemo.equals(dt.getDocBaseType())
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (getC_DocTypeTarget_ID() == 0)
			setC_DocTypeTarget_ID(isSOTrx() ? X_C_DocType.DOCBASETYPE_ARInvoice : X_C_DocType.DOCBASETYPE_APInvoice);
	...
	public static void setIsPaid (Properties ctx, int C_BPartner_ID, String trxName)
	{
		...
		POResultSet<MInvoice> rs = new Query(ctx, I_C_Invoice.Table_Name, whereClause.toString(), trxName)
	...
	private List<Integer> getProductBomLines(int productBomId) {
		return new Query(getCtx(), I_PP_Product_BOMLine.Table_Name, I_PP_Product_BOMLine.COLUMNNAME_PP_Product_BOM_ID+"=?", get_TrxName())
				.setParameters(productBomId)
				.setClient_ID()
				.setOnlyActiveRecords(true)
				.setOrderBy(I_PP_Product_BOMLine.COLUMNNAME_Line)
				.getIDsAsList();
	}	//	getLines
	...
	private X_PP_Product_BOM getDefaultProductBom(MProduct product, String trxName) {
		return (X_PP_Product_BOM) new Query(product.getCtx(), I_PP_Product_BOM.Table_Name, "M_Product_ID=? AND Value=?", trxName)
	...
	private int getPlantForWarehouse(int warehouseId) {
		final String sql = "SELECT MIN("+I_S_Resource.COLUMNNAME_S_Resource_ID+")"
							+" FROM "+I_S_Resource.Table_Name
							+" WHERE "+I_S_Resource.COLUMNNAME_IsManufacturingResource+"=?"
							+" AND "+I_S_Resource.COLUMNNAME_ManufacturingResourceType+"=?"
							+" AND "+I_S_Resource.COLUMNNAME_M_Warehouse_ID+"=?";
		int plantId = DB.getSQLValueEx(null, sql, true, X_S_Resource.MANUFACTURINGRESOURCETYPE_Plant, warehouseId);
		return plantId;
	}
	...
	public String completeIt()
	{
		...
		if (PAYMENTRULE_Cash.equals(getPaymentRule()) && !fromPOS ) {
			if(!OFBForward.OverrideNativeCashLineCreate()) {
				//	Validate Cash book
				if(!isValidCashBook()) {
					processMsg = "@NoCashBook@";
					return DocAction.STATUS_Invalid;
...
//		MOrgInfo counterOrgInfo = MOrgInfo.get(getCtx(), counterAD_Org_ID);
	...
	private String payWithCashAsPayment() {
        ...
        paymentCash.setTenderType(X_C_Payment.TENDERTYPE_Cash);
	...
	private void allocatePrepayments()
	{
		...
		List<MPayment> prepayments = new Query(getCtx(), I_C_Payment.Table_Name, whereClause, get_TrxName())
				.setParameters(getC_Order_ID())
				.setOrderBy(I_C_Payment.COLUMNNAME_DateTrx + ", " + I_C_Payment.COLUMNNAME_C_Payment_ID )
				.list();
