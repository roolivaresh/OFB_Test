...
public class MRequisitionLine extends X_M_RequisitionLine
{
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		if (newRecord
				|| (!newRecord && is_ValueChanged(I_M_RequisitionLine.COLUMNNAME_C_Tax_ID) && !getParent().isProcessed())
				|| (!newRecord && is_ValueChanged(I_M_RequisitionLine.COLUMNNAME_Qty) && !getParent().isProcessed())
				|| (!newRecord && is_ValueChanged(I_M_RequisitionLine.COLUMNNAME_PriceActual) && !getParent().isProcessed())
		) {
			setTaxAmt();
		}
	...
	private boolean updateHeader()
	{
		...
		//jleyton 20220223 se agrega funcion para que tambien se actualice el grandtotal con el impto en la cabecera
		updateHeaderTax();
		if (no != 1)
	...
	private boolean updateHeaderTax()
	{
		...
		if (getParent().isTaxIncluded()) {
			...
			//jleyton 20232023 se corrige update para que tome solo el impto de las lineas asociadas a la solicitud
			final String updateRequisitionGrandTotal = "UPDATE M_Requisition r "
					+ " SET GrandTotal=TotalLines+"
					+ "(SELECT COALESCE(SUM(TaxAmt),0) FROM M_RequisitionTax rt WHERE rt.M_Requisition_ID=r.M_Requisition_ID) "
					+ "WHERE M_Requisition_ID = ? ";
			no = DB.executeUpdateEx(updateRequisitionGrandTotal , List.of(getM_Requisition_ID()).toArray() , get_TrxName());
		}
		if (no != 1)
