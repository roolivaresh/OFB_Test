	...
	//implements MAssetType.Model //commented by @win
{
	...
	{
		return (MAsset)MTable.get(ctx, I_A_Asset.Table_Name).getPO(A_Asset_ID, trxName);
	}	//	get
	...
	public static Collection<MAsset> forASI(Properties ctx, int M_Product_ID, int M_ASI_ID)
	{
		...
		//
		return new Query(ctx, I_A_Asset.Table_Name, whereClause, null)
	...
	protected boolean beforeSave (boolean newRecord)
	{
		...
		//Rodrigo Olivares Hurtado 2023-08-31 se agrega validacion con ofbforward
		if(!OFBForward.IgnorateAssetTSM()) {
			...
			if (getA_Asset_Group_ID() != productCategory.getA_Asset_Group_ID())
			{
...
//		}
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		{
			final String sql = "UPDATE " + I_A_Depreciation_Workfile.Table_Name
				+" SET " + I_A_Depreciation_Workfile.COLUMNNAME_IsDepreciated+"=?"
				+" WHERE " + I_A_Depreciation_Workfile.COLUMNNAME_A_Asset_ID+"=?";
	...
	protected boolean beforeDelete()
	{
		// delete addition
		{
			String sql = "DELETE FROM "+I_A_Asset_Addition.Table_Name+" WHERE "+I_A_Asset_Addition.COLUMNNAME_Processed+"=? AND "+I_A_Asset_Addition.COLUMNNAME_A_Asset_ID+"=?";
		...
		// update invoice line
		{
			final String sql = "UPDATE "+I_C_InvoiceLine.Table_Name+" SET "
										+" "+I_C_InvoiceLine.COLUMNNAME_A_Asset_ID+"=?"
										+","+I_C_InvoiceLine.COLUMNNAME_A_Processed+"=?"
								+" WHERE "+I_C_InvoiceLine.COLUMNNAME_A_Asset_ID+"=?";
