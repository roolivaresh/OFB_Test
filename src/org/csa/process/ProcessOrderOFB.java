 ...
 */
public class ProcessOrderOFB extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		if (p_Order_ID > 0)
		{
			...
			//mfrojas
			String actualStatus = ord.getDocStatus();
			int workflowID = DB.getSQLValue(get_TrxName(), "SELECT coalesce(max(ad_workflow_id),0) from ad_workflow "
					+ " WHERE c_doctype_id = ? and ad_client_id = "+Env.getAD_Client_ID(getCtx()), ord.getC_DocTypeTarget_ID());
