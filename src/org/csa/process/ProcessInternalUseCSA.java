 ...
 *
 *  @author mfrojas
 *  @version $Id: ProcessInternalUseHFBC.java $
 */
public class ProcessInternalUseCSA extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		if (p_Inventory_ID > 0)
		{
			...
			//mfrojas
			String actualStatus = inv.getDocStatus();
			int workflowID = DB.getSQLValue(get_TrxName(), "SELECT coalesce(max(ad_workflow_id),0) from ad_workflow "
					+ " WHERE c_doctype_id = ? and ad_client_id = "+Env.getAD_Client_ID(getCtx()), inv.getC_DocType_ID());
			...
			if(workflowID > 0)
			{
				...
				if(cant > 0)
				{
					if(newStatus.compareTo("CO") != 0)
					{
						...
						{
							//req.voidIt();
							//Validar que el parametro de anulacion este ingresado
						/*	if(p_Message == null)
								throw new AdempiereException("Debe ingresar el mensaje de anulacion");
							else
								req.set_CustomColumn("VoidMessage", p_Message);
						*/
